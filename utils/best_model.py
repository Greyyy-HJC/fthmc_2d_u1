...
class LocalResAttnNetLiteNew(nn.Module):
    """
    Lightweight CNN with residual connection and channel attention.
    ...
    - Input: Concatenated plaquette and rectangle features (6 channels total)
    - Conv1: 6 → 12 channels, 3x3 kernel, circular padding, GELU activation
    - Residual Block: 12 → 12 → 12 channels, two 3x3 convs with residual connection, SiLU activation
    - Channel Attention: Global avg pool → squeeze → expand → sigmoid gating
    - Conv2: 12 → 12 channels, 3x3 kernel, output layer
    - Output: tanh normalization to [-1, 1] range with learnable scaling
    ...
    def __init__(self):
        ...
        # Parameters: 12 * 12 * 3 * 3 + 12 = 1,308
        self.res_conv2 = nn.Conv2d(ch, ch, config.kernel_size, padding='same', padding_mode='circular')
        self.res_activation = nn.SiLU()  # 0 parameters
    ...
    def forward(self, plaq_features, rect_features):
        ...
        # Residual block (2,616 parameters used)
        res = self.res_conv1(x)  # 1,308 parameters
        res = self.res_activation(res)  # 0 parameters
        res = self.res_conv2(res)  # 1,308 parameters
        x = x + 0.3 * res  # 0 parameters - tensor addition
...
class LocalStableNet(nn.Module):
    ...
    def forward(self, plaq_features, rect_features):
        ...
        identity1 = x  # 0 parameters
        x = self.res_block1(x) * 0.3 + identity1  # ~3,600 parameters, moderate scaling
        
        identity2 = x  # 0 parameters
        x = self.res_block2(x) * 0.3 + identity2  # ~3,600 parameters
...
class LocalResNet(nn.Module):
    ...
    - Two standard residual blocks with learnable scaling
    - Output projection: 14 → 12 channels with 1x1 conv
    ...
    def forward(self, plaq_features, rect_features):
        ...
        # two ResNet blocks (~7,200 parameters used)
        ...
        x = self.output_conv(x)  # 180 parameters
        x = torch.arctan(x) / math.pi / 2  # 0 parameters - range limiting
        x = x * self.output_scale  # 1 parameter - learnable scaling
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, channels, kernel_size=(3, 3)):
        ...
        self.alpha = nn.Parameter(torch.tensor(0.5))  # 1 parameter - learnable residual scaling
...
def choose_cnn_model(model_tag):
    ...
    elif model_tag == 'lite_new':
        return LocalResAttnNetLiteNew
    elif model_tag == 'rsat':
