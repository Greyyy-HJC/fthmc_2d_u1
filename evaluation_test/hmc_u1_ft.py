...
from fthmc_2d_u1.utils.func import plaq_from_field, plaq_mean_from_field, regularize, topo_from_field
    ...
    def run(self, n_iterations, theta, store_interval=1, save_config=True):
        ...
        for i in tqdm(range(n_iterations), desc="Running HMC"):
            ...
            if i % store_interval == 0:  # Only store data at specific intervals
                ...
                with torch.no_grad():
                    theta_reg = regularize(theta)
                    theta_ori = regularize(self.field_transformation(theta_reg))
                    if save_config:
                        theta_ori_ls.append(theta_ori.detach().cpu())
                    plaq = plaq_mean_from_field(theta_ori)
                    topo = topo_from_field(theta_ori)
                plaq_buf.append(plaq.detach().cpu())
                ham_buf.append(torch.as_tensor(H_val).detach().cpu())
                topo_buf.append(topo.detach().cpu())
        ...
        plaq_ls = [p.item() for p in plaq_buf]
        hamiltonians = [h.item() for h in ham_buf]
        topological_charges = [t.item() for t in topo_buf]
